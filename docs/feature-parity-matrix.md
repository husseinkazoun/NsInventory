...
| **Tests** | ✅ 69 tests, 209 assertions (per `CLAUDE.md`; not re-run for this branch — no PHP was touched, and PHPUnit is not installed in this checkout) | 🟡 42 Vitest tests (resolver, session expiry, role matrix, Storage) + 84 SQL authorization assertions against a local Postgres 17 cluster; product/order/clothing modules untested | Extend coverage as modules are ported |
...
| # | Gap | Severity | Detail |
|---|---|---|---|
| 1 | ~~**`org_role` is declared but never enforced**~~ | ~~High~~ | Every RLS policy in `0001_initial.sql` / `0002_scans.sql` is a bare `is_org_member(organization_id)`. A `viewer` can insert, update and delete lab assets, scans and missing components exactly like an `owner`. The enum existed purely as a label. **Resolved** by `20260725161641_role_based_rls.sql`, verified by 84 local authorization assertions. Not yet applied to the hosted project. |
| 1b | ~~**No explicit `GRANT`s in the migrations**~~ | ~~Medium~~ | Tables are reachable only because the project predates Supabase's opt-in Data API change (default for projects created after 2026-05-30; this one was created 2026-05-18). **Resolved**: the migration revokes everything then grants exactly the matrix, so exposure no longer depends on project-creation defaults. |
| 1c | ~~**Mutable `search_path` on two functions**~~ | ~~Medium~~ | Live advisors flag `public.set_updated_at` and `public.scan_object_org` as having a role-mutable `search_path`. `is_org_member` already sets it correctly. **Resolved**: both now `set search_path = ''`. |
| 1d | ~~**`is_org_member()` callable via `/rest/v1/rpc` by `anon`**~~ | ~~Low~~ | Confirmed by live advisors, as the Supabase skill predicts for any `SECURITY DEFINER` function in `public`. Low impact — it takes an org id and uses `auth.uid()` internally, so it only reports whether *the caller* is a member and cannot enumerate anyone else. **Resolved**: dropped and replaced by helpers in the unexposed `private` schema. (The advisor also flags `public.rls_auto_enable()`; that is Supabase's own `ensure_rls` event-trigger function and returns `event_trigger`, so it is not actually RPC-callable — a false positive.) |
| 1e | ~~**Per-row `auth.uid()` re-evaluation**~~ | ~~Performance~~ | Live advisors report `auth_rls_initplan` on `profiles_select`, `profiles_self_insert`, `profiles_self_update` and `activity_log_insert`. **Resolved**: policies now use set-returning helpers in `in (select ...)` form, evaluated once per statement. |
| 1f | **Leaked-password protection disabled** | Low | Supabase Auth can reject passwords found in HaveIBeenPwned. Currently off; enable in Auth settings. |
