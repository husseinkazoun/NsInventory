...
| # | Gap | Severity | Detail |
|---|---|---|---|
| 1 | **`org_role` is declared but never enforced** | **High** | Every RLS policy in `0001_initial.sql` / `0002_scans.sql` is a bare `is_org_member(organization_id)`. A `viewer` can insert, update and delete lab assets, scans and missing components exactly like an `owner`. The enum exists purely as a label. Fix before adding real users. **Confirmed live 2026-07-25**: `anon`, `authenticated` and `service_role` all hold full DML grants on all 10 tables, so RLS is the only control. |
| 1b | **No explicit `GRANT`s in the migrations** | Medium | Tables are reachable only because the project predates Supabase's opt-in Data API change (default for projects created after 2026-05-30; this one was created 2026-05-18). Recreating the project would make every table invisible to the client. Add explicit grants. |
| 1c | **Mutable `search_path` on two functions** | Medium | Live advisors flag `public.set_updated_at` and `public.scan_object_org` as having a role-mutable `search_path`. `is_org_member` already sets it correctly. Add `set search_path = public, pg_temp` to both. |
| 1d | **`is_org_member()` callable via `/rest/v1/rpc` by `anon`** | Low | Confirmed by live advisors, as the Supabase skill predicts for any `SECURITY DEFINER` function in `public`. Low impact — it takes an org id and uses `auth.uid()` internally, so it only reports whether *the caller* is a member and cannot enumerate anyone else. Consider moving to an unexposed schema. (The advisor also flags `public.rls_auto_enable()`; that is Supabase's own `ensure_rls` event-trigger function and returns `event_trigger`, so it is not actually RPC-callable — a false positive.) |
| 1e | **Per-row `auth.uid()` re-evaluation** | Performance | Live advisors report `auth_rls_initplan` on `profiles_select`, `profiles_self_insert`, `profiles_self_update` and `activity_log_insert`. Wrap as `(select auth.uid())` so it evaluates once per statement instead of once per row. |
